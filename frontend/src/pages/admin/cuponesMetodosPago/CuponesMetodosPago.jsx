...
const CuponesMetodosPago = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://54.162.148.240:3003/api/admin/cupondescuentos');
        setCupones(response.data.cupondescuentos);
        const responseProveedores = await axios.get('http://54.162.148.240:3003/api/admin/metodopagos');
