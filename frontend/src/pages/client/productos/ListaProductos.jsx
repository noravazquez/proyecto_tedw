...
const ListaProductos = ({ idCategoriaSeleccionada }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                let url = 'http://34.207.241.39:3003/api/product/products'

                if (idCategoriaSeleccionada != null) {
                    url = `http://34.207.241.39:3003/api/product/productsByCategoria/${idCategoriaSeleccionada}`
                    const totalResponse = await axios.get(`http://34.207.241.39:3003/api/product/productoByCategoria/${idCategoriaSeleccionada}`);
                    setTotalProductos(totalResponse.data.totalByCategoria)
                }else{
                    const totalResponse = await axios.get('http://34.207.241.39:3003/api/product/totalProductos');
