...
const ListaProductos = ({ idCategoriaSeleccionada }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                let url = 'http://23.20.161.84:3003/api/product/products'

                if (idCategoriaSeleccionada != null) {
                    url = `http://23.20.161.84:3003/api/product/productsByCategoria/${idCategoriaSeleccionada}`
                    const totalResponse = await axios.get(`http://23.20.161.84:3003/api/product/productoByCategoria/${idCategoriaSeleccionada}`);
                    setTotalProductos(totalResponse.data.totalByCategoria)
                }else{
                    const totalResponse = await axios.get('http://23.20.161.84:3003/api/product/totalProductos');
