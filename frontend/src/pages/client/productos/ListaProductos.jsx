...
const ListaProductos = ({ idCategoriaSeleccionada }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                let url = 'http://54.224.214.208:3003/api/product/products'

                if (idCategoriaSeleccionada != null) {
                    url = `http://54.224.214.208:3003/api/product/productsByCategoria/${idCategoriaSeleccionada}`
                    const totalResponse = await axios.get(`http://54.224.214.208:3003/api/product/productoByCategoria/${idCategoriaSeleccionada}`);
                    setTotalProductos(totalResponse.data.totalByCategoria)
                }else{
                    const totalResponse = await axios.get('http://54.224.214.208:3003/api/product/totalProductos');
