...
const BestSeller = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const productosResponse = await axios.get('http://23.20.161.84:3003/api/product/lastproducts')
        setLastProductos(productosResponse.data.productos)
        const response = await axios.get('http://23.20.161.84:3003/api/admin/categorias')
  ...
  const handleCategoriaClick = async (idCategoria) => {
    try {
      ...
      let url = 'http://23.20.161.84:3003/api/product/lastproducts';
